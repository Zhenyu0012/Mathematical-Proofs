...
Proving two infinite sets have the same cardinality confuses lots of beginners since two sets obviously have different numbers of elements in most cases. For example, $(0,1)\subset[0,1]$, but they have the same cardinality. The short answer<sup>[[1]](https://en.wikipedia.org/wiki/Hilbert%27s_paradox_of_the_Grand_Hotel)</sup> is we use the concept $\infty+n=\infty, n\in\mathbb{N}$. Thus, we can finish our proof if we can define a collection of linear functions satisfying each $a\in A$ maps to precisely one $b\in B$.
